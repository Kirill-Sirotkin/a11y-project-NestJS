...
export class StrategiesVerificationService {
    ...
    async validateUserJwtResetPassword(userId: string, resetPasswordToken: string): Promise<void> {
        const resetToken = await this.databaseService.getResetPasswordTokenByUserId(userId)
        if (!resetToken) {
            throw new NotFoundException('reset password token not found');
        }

        const isResetPasswordTokenValid = await argon2.verify(resetToken.tokenHash, resetPasswordToken);
