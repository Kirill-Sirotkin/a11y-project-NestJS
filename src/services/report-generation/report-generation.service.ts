...
export class ReportGenerationService {
    generateReport(results: AxeResults): jsPDF.jsPDF {
        ...
        page_Y_offset = applyText(doc, page_Y_offset, 0, COLOR_BLACK, FONT_SIZE_LARGE, "bold", "Description & Terminology")
        page_Y_offset = newLine(doc, page_Y_offset)
        page_Y_offset = applyText(doc, page_Y_offset, INDENT_SPACING, COLOR_BLACK, FONT_SIZE_MEDIUM, "normal", 
        ...
        page_Y_offset = applyText(doc, page_Y_offset, INDENT_SPACING, COLOR_BLACK, FONT_SIZE_SMALL, "bold", 
            "More information about WCAG: https://www.w3.org/TR/WCAG22/", "F", COLOR_LIGHT_GREY
        )
        
        page_Y_offset = newLine(doc, page_Y_offset, 1)
        page_Y_offset = applyText(doc, page_Y_offset, 0, COLOR_BLACK, FONT_SIZE_MEDIUM, "bold", "What are violations?")
        page_Y_offset = applyText(doc, page_Y_offset, INDENT_SPACING, COLOR_BLACK, FONT_SIZE_MEDIUM, "normal", 
        ...
        page_Y_offset = applyText(doc, page_Y_offset, 0, COLOR_BLACK, FONT_SIZE_MEDIUM, "bold", "What are failing elements?")
        page_Y_offset = applyText(doc, page_Y_offset, INDENT_SPACING, COLOR_BLACK, FONT_SIZE_MEDIUM, "normal", 
        ...
        page_Y_offset = newLine(doc, page_Y_offset, 0.8)
        page_Y_offset = applyText(doc, page_Y_offset, 0, COLOR_BLACK, FONT_SIZE_MEDIUM, "bold", "What are impact levels?")
        page_Y_offset = applyText(doc, page_Y_offset, INDENT_SPACING, COLOR_BLACK, FONT_SIZE_MEDIUM, "normal", 
        ...
        doc.text(
            ...
            PAGE_HEIGHT - MARGIN_BOTTOM - (FONT_SIZE_SMALL / PPMM) * bottomImpactTextOffset - LINE_SPACING * bottomImpactTextOffset, 
            { align: 'center' }
        )
        doc.text(
            "moderate", 
        ...
        doc.rect( 
            ...
            PAGE_HEIGHT - MARGIN_BOTTOM - (FONT_SIZE_SMALL / PPMM) * bottomImpactColorOffset - LINE_SPACING * bottomImpactColorOffset, 
            colorSquareLength,
            colorSquareLength,
            "F"
        )
        ...
        page_Y_offset = PAGE_HEIGHT - MARGIN_BOTTOM - (FONT_SIZE_SMALL / PPMM) * bottomNoteTextOffset - LINE_SPACING * bottomNoteTextOffset
        page_Y_offset = applyText(doc, page_Y_offset, 0, COLOR_BLACK, FONT_SIZE_SMALL, "bold", 
        ...
        doc.addPage()
        page_Y_offset = MARGIN_TOP
        page_Y_offset = applyText(doc, page_Y_offset, 0, COLOR_BLACK, FONT_SIZE_MEDIUM, "bold", `Total violations: ${results.violations.length}`)
        results.violations.forEach((violation, violation_i) => {
            ...
            const impactText = color === COLOR_BLACK ? "not specified" : violation.impact
            ...
            violation.nodes.forEach((node, node_i) => {
                ...
                const impactText = color === COLOR_BLACK ? "not specified" : node.impact
                page_Y_offset = applyText(
                    doc, 
                    page_Y_offset, 
                    INDENT_SPACING * 2, 
                ...
                const summaryTextRaw = node.failureSummary ? node.failureSummary : "not specified"
...
function applyText(
    ...
    switch(fill) {
        ...
        case "F":
            ...
            const width = getLongestWidthOfStringFromArray(doc, splitText) + HIGHLIGHT_MARGIN_RIGHT
            const height = ((fontSize / PPMM) * splitText.length + LINE_SPACING * (splitText.length - 1)) + HIGHLIGHT_MARGIN_BOTTOM
            doc.rect(x_indent + MARGIN_LEFT - HIGHLIGHT_MARGIN_LEFT, y_offset, width, height, "F")
        default:
            doc.setFillColor(COLOR_WHITE)
    }
...
function getLongestWidthOfStringFromArray(doc: jsPDF.jsPDF, array: string[]): number {
    let length = 0
...
function newLine(doc: jsPDF.jsPDF, y_offset: number, multiplier: number = 1): number {
...
function checkOffsetOverflow(y_offset: number, linesCount: number, fontSize: number): boolean {
...
function getFontColorFromImpactValue(impact: axe.ImpactValue | undefined): string {
