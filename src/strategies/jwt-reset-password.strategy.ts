...
export class JwtResetPasswordStrategy extends PassportStrategy(Strategy, "jwt-reset-password") {
  constructor(private readonly strategiesVerificationService: StrategiesVerificationService) {
    ...
    if (jwtSecret === undefined) {
      throw new Error('JWT_RESET_PASSWORD_SECRET is not defined in the environment variables');
    }
  ...
  async validate(req: Request, payload: JwtVerifyPayloadDto): Promise<JwtVerifyPayloadDto> {
    ...
    await this.strategiesVerificationService.validateUserJwtResetPassword(payload.sub, resetPasswordToken)
    return payload;
  }
}